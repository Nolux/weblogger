    ...
    {:else}
      ...
      >
        ...
        {#each $localLtcState.devices as device}
          <option value={device.deviceId}>{device.label || "Audio input"}</option>
        {/each}
      </select>
      <div class="flex gap-2">
        ...
        >Connect</button>
        <button class="btn flex-1" onclick={() => disconnectLocalLtc()}>Disconnect</button>
      </div>
      <div class="text-sm">
      ...
      <p class="text-xs opacity-70">
        Browser audio input requires localhost or HTTPS and user permission. LTC date is not
        carried in the signal; weblogger uses the browser/project day for the log date.
      </p>
    {/if}
  </div>
